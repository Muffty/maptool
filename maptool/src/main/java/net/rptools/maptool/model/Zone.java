...
public class Zone extends BaseModel {
    ...
    public void androidPutToken(AndroidToken androidToken) {
        boolean newToken = !tokenMap.containsKey(androidToken.id);
        if (newToken) {
            log.error("Tried to put AndroidToken" + androidToken + " for which there is no normal Token present");
            return;
        }
        Token oldToken = getToken(androidToken.id);
        Token token = oldToken.newTokenFromThisAndAndroidToken(androidToken);
        tokenMap.put(token.getId(), token);
